...
#if !defined(_KALIBRI_CLASSTYPE_H_)
#define _KALIBRI_CLASSTYPE_H_
...
// Every Squirrel class object created by kalibri in every VM has its own unique ClassData object stored in the registry table of the VM
template<class C>
struct ClassData {
    HSQOBJECT classObj;
    HSQOBJECT getTable;
...
public:
    ...
    static KALIBRI_API WeakPtr<AbstractStaticClassData>& _getStaticClassData(const std::type_info* type);
#else
    ...
    static WeakPtr<AbstractStaticClassData>& _getStaticClassData(const std::type_info* type) {
...
public:
    ...
    static inline AbstractStaticClassData*& BaseClass() {
        assert(getStaticClassData().Expired() == false); // fails because called before a kb::Class for this type exists
