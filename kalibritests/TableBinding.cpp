...
TEST_F(KalibriTest, SimpleTableBinding)
{
    DefaultVM::Set(vm);
...
TEST_F(KalibriTest, TableCleanup)    // test case for Sourceforge kalibri Bug 43
{
    static const SQChar *sq_code = _SC("\
        local i; \
        for (i = 0; i < 12; i++) \
    ...
    if (kb::Error::Occurred(v)) {
        FAIL() << _SC("Compile Failed: ") << kb::Error::Message(v);
    }

    script.Run();
    if (kb::Error::Occurred(v)) {
        FAIL() << _SC("Run Failed: ") << kb::Error::Message(v);
    }
    const int length = 12;
...
void touch_element(kb::Table & t, const char *key, int val) 
{
    t.SetValue(key, val);        
}

void touch_element2(kb::Table t, const char *key, int val) 
{
    t.SetValue(key, val);        
}


void touch_element3(kb::Table t, int index, kb::Table &t2) 
{
    t.SetValue(index, t2);        
}

void touch_element4(kb::Table t, const char *key, kb::Table &t2) 
{
    t.SetValue(key, t2);        
}

TEST_F(KalibriTest, PassingTableIn) {
    char buf[200];
...
TEST_F(KalibriTest, PassingTableIn2) {
    static const int SIZE = 56;
    static const SQChar *sq_code = _SC("\
        local i; \
        local t2 = {} \
...
TEST_F(KalibriTest, PassingTableIn3) {
    static const int SIZE = 56;
    static const SQChar *sq_code = _SC("\
