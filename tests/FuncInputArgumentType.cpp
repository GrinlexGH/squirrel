...
TEST_F(KalibriTest, NumericArgumentTypeConversionAndCheck) {
    DefaultVM::Set(vm);
    
    kb::Class<Vector2> classVector2(vm, _SC("Vector2"));
    ...
    kb::RootTable(vm).Bind(_SC("Vector2"), classVector2);
...
TEST_F(KalibriTest, FunctionOfSameNumberOfArgumentsButDifferentTypesBinding) {
    DefaultVM::Set(vm);
    
    kb::Class<F> Fclass(vm, _SC("F"));
    Fclass.Func<int (F::*)(int, char)>(_SC("f1"), &F::func);
    Fclass.Func<const char * (F::*)(char, const char*)>(_SC("f2"), &F::func);
    
    kb::RootTable(vm).Bind(_SC("F"), Fclass);
